...
let
  ...
  options = {
    ...
    tracing = {
      ...
      SCHED_TRACER          = yes;
      STACK_TRACER          = yes;
      UPROBE_EVENT          = whenOlder "4.11" (option yes);
      UPROBE_EVENTS         = whenAtLeast "4.11" (option yes);
    ...
    tests = {
      ...
      DRM_DEBUG_MM_SELFTEST    = whenOlder "4.18" (option no);
      LNET_SELFTEST            = whenOlder "4.18" (option no);
